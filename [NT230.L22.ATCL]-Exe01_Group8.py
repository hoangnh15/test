...
def encrypt_Payload(payload, address, length_of_payload):
    enc = []
    for i in range(0, length_of_payload, 4):
        tmp = int(payload[i:i+4].hex(), 16).to_bytes(4, byteorder='little', signed=False)
        enc.append(int(tmp.hex(), 16)^(address+i)^(length_of_payload-i))
